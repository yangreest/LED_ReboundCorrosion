...
//                            LogHelper.getInstance().e("delete folder :" + file.getAbsolutePath());
    ...
    /**
     * 将URI转成绝对路径
     * 注意：在 Android 10+ 上，由于分区存储限制，部分 URI 可能无法转换为路径
     * 建议使用 ContentResolver 直接操作 URI
     *
     * @param context 上下文对象
     * @param uri     URI
     * @return 绝对路径，如果无法获取则返回 null
     */
    public static String convertUriToPath(Context context, Uri uri) {
        ...
        if (scheme == null) {
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    if (displayNameIndex != -1) {
                        ...
                        // 对于 Android 10+，尝试使用缓存目录
                        File cacheFile = new File(context.getCacheDir(), displayName);
                        path = cacheFile.getAbsolutePath();
                    }
                }
            } catch (Exception e) {
                LogHelper.getInstance().e("convertUriToPath error: " + e.toString());
            } finally {
                if (cursor != null) {
                    cursor.close();
                }
            ...
            // 对于 Android 9 及以下，尝试使用 DATA 列
            if (path == null && android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.Q) {
                try {
                    ...
                    if (dataCursor != null) {
                        if (dataCursor.moveToFirst()) {
                            int index = dataCursor.getColumnIndex(MediaStore.Images.ImageColumns.DATA);
                            if (index > -1) {
                                path = dataCursor.getString(index);
                            }
                        }
                        dataCursor.close();
                    }
                } catch (Exception e) {
                    LogHelper.getInstance().e("convertUriToPath DATA column error: " + e.toString());
                }
            }
        }
        return path;
