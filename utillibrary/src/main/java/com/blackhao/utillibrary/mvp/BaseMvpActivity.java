...
public abstract class BaseMvpActivity<T extends BaseMvpPresenterImpl> extends AppCompatActivity
    ...
    public void replaceFragment(@IdRes int frameId, Fragment showFragment) {
        replaceFragment(frameId, showFragment, false);
    }
     ...
     */
    public void replaceFragment(@IdRes int frameId, Fragment showFragment, boolean addToBackStack) {
        ...
        if (currentFragment != null && currentFragment != showFragment) {
            if (!showFragment.isAdded()) {    // 先判断是否被add过
                transaction.hide(currentFragment).add(frameId, showFragment); // 隐藏当前的fragment，add下一个到Activity中
            } else {
                transaction.hide(currentFragment).show(showFragment); // 隐藏当前的fragment，显示下一个
            }
            if (addToBackStack) {
            ...
            if (!showFragment.isAdded()) {    // 先判断是否被add过
                transaction.add(frameId, showFragment); // 隐藏当前的fragment，add下一个到Activity中
            } else {
                transaction.show(showFragment); // 隐藏当前的fragment，显示下一个
            }
            if (addToBackStack) {
                transaction.addToBackStack(null);
            }
