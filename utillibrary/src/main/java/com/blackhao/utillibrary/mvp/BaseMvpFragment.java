...
public abstract class BaseMvpFragment<T extends BaseMvpPresenterImpl> extends Fragment implements BaseMvpViewImpl {
    ...
    //Activity（弱引用防止内存泄漏）
    private WeakReference<BaseMvpActivity> activityRef;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        // 使用 instanceof 检查类型安全
        if (getActivity() instanceof BaseMvpActivity) {
            activityRef = new WeakReference<>((BaseMvpActivity) getActivity());
            log = ((BaseMvpActivity) getActivity()).log;
        }
     ...
     */
    @Nullable
    protected BaseMvpActivity getActivityRef() {
    ...
    public void showToast(final String str) {
        BaseMvpActivity activity = getActivityRef();
        if (activity != null) {
