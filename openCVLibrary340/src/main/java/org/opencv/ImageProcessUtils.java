...
public class ImageProcessUtils {
    ...
    /**
     * 合并方法：高斯模糊 -> 二值化阈值处理 -> 高斯模糊
     * 相比分开调用三次方法，减少了 Bitmap/Mat 转换次数，提升性能
     */
    public static void gaussianBlurThresholdBlur(Bitmap bitmap) {
        org.opencv.android.Utils.bitmapToMat(bitmap, sSrc);
        // 1. 高斯模糊 (5x5)
        Imgproc.GaussianBlur(sSrc, sDst, new Size(5, 5), 0, 0, 4);
        // 2. 灰度转换
        Imgproc.cvtColor(sDst, sDst, Imgproc.COLOR_BGRA2GRAY);
        // 3. 二值化阈值处理 (OTSU)
        Imgproc.threshold(sDst, sDst, 0, 255, Imgproc.THRESH_BINARY | Imgproc.THRESH_OTSU);
        // 4. 再次高斯模糊 (5x5)
        Imgproc.GaussianBlur(sDst, sDst, new Size(5, 5), 0, 0, 4);
        org.opencv.android.Utils.matToBitmap(sDst, bitmap);
        sSrc.release();
        sDst.release();
    }
