...
import com.example.gtj_f230_rebound_corrosion.base.activity.CrashMessageActivity;
...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private void saveCrashInfo2File(Throwable ex) {
        ...
        sb.append(result);
        sendEmail(result);
        if (StaticConstant.CRASH_DEBUG) {
            try {
                long timestamp = System.currentTimeMillis();
                String time = formatter.format(new Date());
                String fileName = "crash-" + time + "-" + timestamp + ".txt";
                if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    String path = StaticConstant.crashFilePath;
    ...
    private void sendEmail(String result) {
        Intent it = new Intent(mContext, CrashMessageActivity.class);
