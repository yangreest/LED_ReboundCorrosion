...
public class TriangleRightView extends View {
    ...
    public TriangleRightView(Context context, boolean isFocus, float currentY) {
        super(context);
    ...
    public TriangleRightView(Context context, boolean isFocus, float currentX, float currentY) {
        super(context);
        this.isFocus = isFocus;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        path.lineTo((float) a.x, (float) a.y);
        path.close();
        paint.setColor(Color.parseColor("#E66163"));
        ...
        if (isFocus) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    if (Math.abs(event.getX() - currentX) < 56
                            && Math.abs(event.getY() - currentY) < 56) {
                        isEfficient = true;
                        currentX = event.getX();
                        currentY = event.getY();
                    }
