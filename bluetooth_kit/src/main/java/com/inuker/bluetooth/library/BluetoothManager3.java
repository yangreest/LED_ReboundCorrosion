...
public class BluetoothManager3 {
    ...
    public static void init(Context context, String USR_SERVICE, String USR_CHARACTER_NOTIFY, String USR_CHARACTER_WRITE) {
        BluetoothManager3.USR_SERVICE = UUID.fromString(USR_SERVICE);
        BluetoothManager3.USR_CHARACTER_NOTIFY = UUID.fromString(USR_CHARACTER_NOTIFY);
        BluetoothManager3.USR_CHARACTER_WRITE = UUID.fromString(USR_CHARACTER_WRITE);
    ...
    public static BluetoothClient getClient() {
        if (mClient == null) {
            synchronized (BluetoothManager3.class) {
                if (mClient == null) {
    ...
    public static void searchDeviceDevice(SearchResponse mSearchResponse) {
        ...
        BluetoothManager3.getClient().search(request, mSearchResponse);
    }

    public static boolean getConnectStatus(String mac) {
        return 2 == BluetoothManager3.getClient().getConnectStatus(mac);
    }

    public static void connectBluetoothDevice(final String mac, final StatusResult statusResult) {
        BluetoothManager3.getClient().connect(mac, (code, data) -> {
    ...
    public static void unConnect(String mac) {
        ...
        //断开设备连接
        BluetoothManager3.getClient().disconnect(mac);
    }
    ...
    public static void notifyBluetoothDevice(String mac, BleNotifyResponse bleNotifyResponse) {
        new Handler(Looper.getMainLooper()).postDelayed(() -> BluetoothManager3.getClient().notify(mac, USR_SERVICE, USR_CHARACTER_NOTIFY, bleNotifyResponse), 500);
    }

    public static void unNotifyBluetoothDevice(String mac) {
        BluetoothManager3.getClient().unnotify(mac, USR_SERVICE, USR_CHARACTER_NOTIFY, code -> {
        });
    }

    private static void writeBluetoothDevice(String mac, byte[] bytes, BleWriteResponse bleWriteResponse) {
        BluetoothManager3.getClient().write(mac, USR_SERVICE, USR_CHARACTER_WRITE, bytes, bleWriteResponse);
    }

    public static void registerConnectStatusListener(String mac, BleConnectStatusListener listener) {
        BluetoothManager3.getClient().registerConnectStatusListener(mac, listener);
    }

    public static void unregisterConnectStatusListener(String mac, BleConnectStatusListener listener) {
        BluetoothManager3.getClient().unregisterConnectStatusListener(mac, listener);
    }
}
