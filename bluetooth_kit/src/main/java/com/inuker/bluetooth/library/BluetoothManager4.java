...
public class BluetoothManager4 {
    ...
    public static void init(Context context, String USR_SERVICE, String USR_CHARACTER_NOTIFY, String USR_CHARACTER_WRITE) {
        BluetoothManager4.USR_SERVICE = UUID.fromString(USR_SERVICE);
        BluetoothManager4.USR_CHARACTER_NOTIFY = UUID.fromString(USR_CHARACTER_NOTIFY);
        BluetoothManager4.USR_CHARACTER_WRITE = UUID.fromString(USR_CHARACTER_WRITE);
    ...
    public static BluetoothClient getClient() {
        if (mClient == null) {
            synchronized (BluetoothManager4.class) {
                if (mClient == null) {
    ...
    public static void searchDeviceDevice(SearchResponse mSearchResponse) {
        ...
        BluetoothManager4.getClient().search(request, mSearchResponse);
    }

    public static boolean getConnectStatus(String mac) {
        return 2 == BluetoothManager4.getClient().getConnectStatus(mac);
    }

    public static void connectBluetoothDevice(final String mac, final StatusResult statusResult) {
        BluetoothManager4.getClient().connect(mac, (code, data) -> {
    ...
    public static void unConnect(String mac) {
        ...
        //断开设备连接
        BluetoothManager4.getClient().disconnect(mac);
    }
    ...
    public static void notifyBluetoothDevice(String mac, BleNotifyResponse bleNotifyResponse) {
        new Handler(Looper.getMainLooper()).postDelayed(() -> BluetoothManager4.getClient().notify(mac, USR_SERVICE, USR_CHARACTER_NOTIFY, bleNotifyResponse), 500);
    }

    public static void unNotifyBluetoothDevice(String mac) {
        BluetoothManager4.getClient().unnotify(mac, USR_SERVICE, USR_CHARACTER_NOTIFY, code -> {
        });
    }

    private static void writeBluetoothDevice(String mac, byte[] bytes, BleWriteResponse bleWriteResponse) {
        BluetoothManager4.getClient().write(mac, USR_SERVICE, USR_CHARACTER_WRITE, bytes, bleWriteResponse);
    }

    public static void registerConnectStatusListener(String mac, BleConnectStatusListener listener) {
        BluetoothManager4.getClient().registerConnectStatusListener(mac, listener);
    }

    public static void unregisterConnectStatusListener(String mac, BleConnectStatusListener listener) {
        BluetoothManager4.getClient().unregisterConnectStatusListener(mac, listener);
    }
}
