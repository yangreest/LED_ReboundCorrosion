...
public class BluetoothUtils {
    ...
    private static void registerGlobalReceiver(BroadcastReceiver receiver, IntentFilter filter) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            // Android 14+ (API 34+): 必须明确指定 RECEIVER_NOT_EXPORTED 或 RECEIVER_EXPORTED
            // 这些是应用内部使用的广播接收器，使用 RECEIVER_NOT_EXPORTED
    ...
    private static void sendGlobalBroadcast(Intent intent) {
        Context context = getContext();
        // Android 13+: 使用 RECEIVER_NOT_EXPORTED 注册的接收器只能接收显式广播
        // 设置 package 使其成为显式广播
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
